...
class Board(Canvas):
    ...
    def loadImages(self):
        ...
        try:
            self.idot = Image.open("../solo_snake_tkinter2/dot.png")
            self.dot = ImageTk.PhotoImage(self.idot)
            self.ihead = Image.open("../solo_snake_tkinter2/head2.png")
            self.head = ImageTk.PhotoImage(self.ihead)
            self.iapple = Image.open("../solo_snake_tkinter2/apple.png")
    ...
    def checkCollisions(self):
        ...
        # Get the bounding box of the head and the coordinates of all dots
        head_bbox = self.bbox("head")
        dot_coords = self.coords("dot")

        # Check if the head has collided with any dots
        if head_bbox in dot_coords:
            self.inGame = False
            return

        # Check if the head has collided with the walls of the game board
        if any(coord < 0 or coord > limit for coord, limit in
               zip(head_bbox, (Cons.BOARD_WIDTH, Cons.BOARD_HEIGHT))):
            self.inGame = False
            return

        # Check if the head has collided with any other part of the snake
        if head_bbox in dot_coords[::-1][1:]:
            self.inGame = False
            return
